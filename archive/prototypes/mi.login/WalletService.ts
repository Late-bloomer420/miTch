...
async function fetchVerifierPublicKey(did: string): Promise<CryptoKey> {
  ...
  const key = await crypto.subtle.importKey('jwk', vm.publicKeyJwk, algorithm, true, [
    'encrypt',
    'wrapKey',
  ]);
...
export class WalletService {
        ...
        if (predicate.startsWith('age >=')) {
          ...
          try {
            ...
            if (result.allPassed) {
              ...
              logs.push(
                `🛡️ ZKP Proof Generated: ${result.decisionId} (${result.binding.requestHash.substring(0, 8)}...)`
              );
            } else {
  ...
  async signData(payload: ProofOfExistence): Promise<{ proofToken: string; auditLog: string[] }> {
    ...
    const auditKeys = (this.auditLog as any).privateKey
      ? {
          privateKey: (this.auditLog as any).privateKey,
          publicKey: (this.auditLog as any).publicKey,
        }
      : null;
