...
**Date:** 2026-06-21 (pointer refresh after H-10 + G-140 PR1 merges)
...
**Current master:** `1d0264d` (PR #112 merged) — *#104 at the H-10 snapshot; since then merged: #110 (H-10 docs), #109 (EU-boundaries/Layer-2 foundation), #108 (dependabot), #106 (4-window launcher), #112 (G-140 Layer-2 PR1). This doc-alignment commit itself lands one PR later.*
...
### Recent additions (since PR #81 → master `1d0264d`/PR #112, 2026-06-08 … 2026-06-21)
...
- **G-140 / Layer-2 Visibility PR1 landed (#112):** `WalletService.evaluateRequest()` now appends a neutral, PII-minimal `POLICY_EVALUATED` audit event on **every** verdict (ALLOW/PROMPT/DENY) carrying `requested_claims` (all raw, incl. over-asked), `authorized_claims`, `denied_claims (= requested − authorized)`, `verdict`, `reason_codes`. New typed `DisclosureDecisionMetadata` in `@askmi/shared-types` (claim names only, never values). Fixes gap A (withheld measured vs raw request) + gap B (DENY now emits an event). TDD; wallet-pwa 157, shared-types 82, full suite 46/46 turbo tasks. **PR1 is the logging substrate only — G-140.1's user-facing requested/allowed/withheld *display* is still open (PR2–PR4 per [`docs/_core/09_LAYER2_VISIBILITY_PLAN.md`](docs/_core/09_LAYER2_VISIBILITY_PLAN.md)).**
