...
export function mergeStates(
  ...
  const allDevices = new Set([...Object.keys(local.counters), ...Object.keys(remote.counters)]);
  ...
  for (const deviceId of allDevices) {
    ...
    counters[deviceId] = Math.max(local.counters[deviceId] ?? 0, remote.counters[deviceId] ?? 0);
