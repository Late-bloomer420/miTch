...
describe('iCloudAdapter (pre-built stub)', () => {
  ...
  it('isAuthenticated() → false', async () =>
    expect(await iCloudAdapter.isAuthenticated()).toBe(false));
...
describe('googleDriveAdapter (pre-built stub)', () => {
  ...
  it('displayName is "Google Drive"', () =>
    expect(googleDriveAdapter.displayName).toBe('Google Drive'));
  it('isAvailable() → false', async () =>
    expect(await googleDriveAdapter.isAvailable()).toBe(false));
  it('isAuthenticated() → false', async () =>
    expect(await googleDriveAdapter.isAuthenticated()).toBe(false));
  it('authenticate() → throws', async () => {
