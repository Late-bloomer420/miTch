...
describe('T-28: Policy Denial & Recovery (DenialResolver)', () => {
  it('should map UNKNOWN_VERIFIER to critical severity with actions', () => {
    const resolution = DenialResolver.resolve(ReasonCode.UNKNOWN_VERIFIER, {
      verifierId: 'did:example:evil-corp',
    });
  ...
  it('should perform string interpolation in messages', () => {
    const resolution = DenialResolver.resolve(ReasonCode.UNKNOWN_VERIFIER, {
      verifierId: 'TargetVerifier',
    });
    expect(resolution.message).toBe(
      'Dieser Service (TargetVerifier) ist nicht im Register. Fortfahren auf eigenes Risiko.'
    );
  });
