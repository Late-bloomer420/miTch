...
const SECURITY_CRITICAL_FLAGS: Array<keyof CapabilityFlags> = [
  'layer0',
  'revocation-online',
  'replay-protection',
];
...
export function negotiateCapabilities(
  ...
  if (input.requestedProfile) {
    for (const [flag, value] of Object.entries(input.requestedProfile) as Array<
      [keyof CapabilityFlags, boolean]
    >) {
  ...
  return {
    ...
    agreedCapabilities: {
      ...
      'revocation-online':
        input.wallet.capabilities['revocation-online'] &&
        input.verifier.capabilities['revocation-online'],
      'revocation-offline':
        input.wallet.capabilities['revocation-offline'] &&
        input.verifier.capabilities['revocation-offline'],
      'replay-protection':
        input.wallet.capabilities['replay-protection'] &&
        input.verifier.capabilities['replay-protection'],
