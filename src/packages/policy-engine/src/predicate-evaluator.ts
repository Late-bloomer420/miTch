import { LegacyPredicateRequest, LegacyDecisionProof, legacyToDSL } from '@mitch/shared-types';
...
export class PredicateEvaluator {
  ...
  static verifyProofBinding(
    ...
    if (proof.commitment.verifierDid !== request.verifierDid) {
      return {
        valid: false,
        reason: `Verifier DID mismatch: expected ${request.verifierDid}, got ${proof.commitment.verifierDid}`,
      };
    }
    if (proof.commitment.nonce !== request.nonce) {
      return {
        valid: false,
        reason: `Nonce mismatch: expected ${request.nonce}, got ${proof.commitment.nonce}`,
      };
    }
    ...
    try {
      ...
      return {
        valid: false,
        reason: `Hashing error: ${err instanceof Error ? err.message : String(err)}`,
      };
    }
