...
import { canonicalStringify, sha256, verifyPairwiseDIDProof } from '@mitch/shared-crypto';
...
describe('U-05: Pairwise DID in Policy Engine Proof Generation', () => {
  ...
  it('DecisionCapsule policy_hash binds to the full PolicyManifest and rule_hash binds to the matched rule', async () => {
    ...
    const result = await engine.evaluate(makeRequest(), makeContext(), [makeCredential()], policy);

    const expectedPolicyHash = await sha256(canonicalStringify(policy));
    const expectedRuleHash = await sha256(canonicalStringify(policy.rules[0]));

    expect(result.verdict).toBe('ALLOW');
    expect(result.decisionCapsule!.policy_hash).toBe(expectedPolicyHash);
    expect(result.decisionCapsule!.rule_hash).toBe(expectedRuleHash);
    expect(result.decisionCapsule!.policy_hash).not.toBe(expectedRuleHash);
  });
  ...
  it('pairwise_proof is a valid ECDSA signature over decision_id', async () => {
    ...
    const sigBytes = new Uint8Array(proofHex.match(/.{2}/g)!.map((h) => parseInt(h, 16)));
  ...
  it('fails closed when pairwise DID generation fails for an ALLOW proof flow', async () => {
    const failingEngine = new PolicyEngine(undefined, async () => {
      throw new Error('pairwise key generation unavailable');
    });

    const result = await failingEngine.evaluate(
      makeRequest(),
      makeContext(),
      [makeCredential()],
      makePolicy()
    ...
    expect(result.verdict).toBe('DENY');
    expect(result.reasonCodes).toContain(ReasonCode.PAIRWISE_DID_FAILED);
    expect(result.decisionCapsule).toBeDefined();
    expect(result.decisionCapsule!.pairwise_did).toBeUndefined();
    expect(result.decisionCapsule!.authorized_requirements).toEqual([]);
  });
});
