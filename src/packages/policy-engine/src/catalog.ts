...
export class DenialResolver {
  ...
  private static mapReasonToDenialCode(reason: string): PolicyDenialCode {
    switch (reason) {
      case ReasonCode.UNKNOWN_VERIFIER:
        return 'UNKNOWN_VERIFIER';
      case ReasonCode.NO_SUITABLE_CREDENTIAL:
        return 'NO_SUITABLE_CREDENTIAL';
      case ReasonCode.CLAIM_NOT_ALLOWED:
        return 'ATTRIBUTE_BLOCKED';
      case ReasonCode.LAYER_VIOLATION:
        return 'LAYER_VIOLATION';
      case ReasonCode.CREDENTIAL_EXPIRED:
      case ReasonCode.CREDENTIAL_TOO_OLD:
        return 'FRESHNESS_EXPIRED';
      case ReasonCode.NO_MATCHING_RULE:
        return 'NO_MATCHING_RULE';
      case ReasonCode.CONSENT_REQUIRED:
        return 'CONSENT_REQUIRED';
      case ReasonCode.UNTRUSTED_ISSUER:
        return 'UNTRUSTED_ISSUER';
      case ReasonCode.PAIRWISE_DID_FAILED:
  ...
  private static CATALOG: Record<PolicyDenialCode, PolicyDenialResolution> = {
    UNKNOWN_VERIFIER: {
      ...
      message:
        'Dieser Service ({verifierId}) ist nicht im Register. Fortfahren auf eigenes Risiko.',
      severity: 'CRITICAL',
    ...
    LAYER_VIOLATION: {
      ...
      message:
        'Dieser Service ist nicht für diese Datenkategorie autorisiert. Die angeforderten Daten erfordern eine höhere Schutzebene.',
      severity: 'CRITICAL',
      learnMoreUrl: 'https://mitch.example/policies#LAYER_MODEL',
      actions: [
        {
          id: 'act_learn',
          label: 'Schutzebenen verstehen',
          type: 'LEARN_MORE',
          requiresConfirm: false,
        },
        {
    ...
    NO_MATCHING_RULE: {
      ...
      actions: [{ id: 'act_support', label: 'Hilfe', type: 'LEARN_MORE', requiresConfirm: false }],
    },
    CONSENT_REQUIRED: {
    ...
    CLAIM_NOT_ALLOWED: {
      ...
      severity: 'HIGH',
      actions: [
        { id: 'act_contact', label: 'Kontakt', type: 'CONTACT_VERIFIER', requiresConfirm: false },
      ],
    },
    CREDENTIAL_EXPIRED: {
      // Aliased
      ...
      severity: 'WARN',
      actions: [
        { id: 'act_renew', label: 'Erneuern', type: 'LOAD_CREDENTIAL', requiresConfirm: false },
      ],
    },
  };
}
