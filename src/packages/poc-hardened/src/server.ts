...
const server = createServer(async (req, res) => {
    ...
    return sendJson(
      ...
      200,
      {
        ...
        endpoints: [
          ...
          'GET /kpi',
          'GET /audit/verify',
          'GET /metrics/reset (ALLOW_DEV_RESET=1)',
          'GET /test-request (LOCAL_TEST_KEYS=1)',
          'POST /verify',
          'POST /override',
          'POST /adjudicate',
        ],
  ...
  if (req.method === 'GET' && req.url === '/dashboard') {
    ...
    const costSavingRate =
      modeledBaselineCost > 0 ? costSavingPerVerification / modeledBaselineCost : 0;
    ...
    const denyBars = Object.entries(m.denyByCode)
      ...
      .map(([code, count]) => {
        const width =
          m.totals.deny > 0 ? Math.max(3, Math.round((count / m.totals.deny) * 100)) : 0;
    ...
    const recentRows = m.recentDecisions
      .map(
        (d) =>
          `<tr><td>${d.at}</td><td>${d.requestId}</td><td>${d.decision}</td><td>${d.decisionCode}</td></tr>`
      )
      .join('');
      ...
      appendEvent({
        ...
        requestId:
          typeof parsed === 'object' && parsed && 'requestId' in parsed
            ? String((parsed as { requestId?: string }).requestId ?? 'unknown')
            : 'unknown',
        rpId:
          typeof parsed === 'object' && parsed && 'rp' in parsed
            ? String((parsed as { rp?: { id?: string } }).rp?.id ?? 'unknown')
            : 'unknown',
      });
