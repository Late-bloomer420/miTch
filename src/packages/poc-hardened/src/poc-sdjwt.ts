...
import { StatusListPublisher, checkRevocation } from './credential/statusList';
...
async function main(): Promise<void> {
  ...
  detail(
    `NOT disclosing: ${credential.disclosures
      .filter((d) => d.claimName !== 'over_18')
      .map((d) => d.claimName)
      .join(', ')}`
  );
  ...
  detail(
    `Minimized: ${minimized.disclosures.length} disclosure(s) sent (encoded only, no metadata)`
  );
  ...
  detail(
    `Real disclosures: ${presentation.disclosedItems.length}, padded to: ${paddedDisclosures.length}`
  );

  // Identical decline
  const decline = declineResponse('req-456');
  detail(
    `Decline response: ${JSON.stringify({ decision: decline.decision, code: decline.decisionCode })}`
  );
