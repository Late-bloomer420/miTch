...
async function run(): Promise<void> {
  ...
  try {
    ...
    assert.ok(
      telemetry.resolver_queries_total >= 2,
      'Expected resolver query telemetry increments'
    );
    assert.ok(
      telemetry.resolver_inconsistent_responses_total >= 1,
      'Expected inconsistency telemetry increment'
    );
    assert.ok(
      telemetry.resolver_quorum_failures_total >= 1,
      'Expected quorum failure telemetry increment'
    );
