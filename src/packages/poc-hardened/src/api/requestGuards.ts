...
export function validateRequestSemantics(
  ...
  if (
    !r.binding ||
    typeof r.binding.nonce !== 'string' ||
    typeof r.binding.requestHash !== 'string'
  ) {
    return { ok: false, code: 'DENY_SCHEMA_MISSING_FIELD' };
  }
  ...
  if (
    !r.proofBundle ||
    typeof r.proofBundle.format !== 'string' ||
    typeof r.proofBundle.alg !== 'string'
  ) {
    return { ok: false, code: 'DENY_SCHEMA_MISSING_FIELD' };
  }

  if (
    r.rp.jurisdiction !== undefined &&
    (typeof r.rp.jurisdiction !== 'string' || r.rp.jurisdiction.length === 0)
  ) {
    return { ok: false, code: 'DENY_SCHEMA_TYPE_MISMATCH' };
  }
