...
export async function verifyProofBundle(
  ...
  if (!bundle.proof || bundle.proof.trim().length === 0)
    return { ok: false, reason: 'empty_proof' };
  const allowedAlgs = getAllowedAlgs();
  if (!bundle.alg || !allowedAlgs.has(bundle.alg)) return { ok: false, reason: 'unsupported_alg' };

  const credentialStatus = await checkCredentialRevocation(
    bundle.credentialId,
    bundle.credentialStatus
  );
  ...
  if (key.status === 'resolver_quorum_failed')
    return { ok: false, reason: 'resolver_quorum_failed' };
