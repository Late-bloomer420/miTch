...
export function generateTransparencyReport(
  ...
  return {
    ...
    whatWeProcess: [
      {
        ...
        retention:
          'Crypto-shredded immediately after credential issuance. Predicate only in signed credential (held by user).',
      },
      {
    ...
    whatWeLog: [
      {
        item: 'Aggregate verification count',
        detail: "e.g., '47 verifications this hour' — no identifiers",
      },
    ...
    userRights: [
      {
        ...
        mitchImplementation:
          "All data stored locally on user's device. Full visibility in wallet UI.",
      },
      {
        ...
        mitchImplementation:
          'Crypto-shredding: encryption key destroyed = data mathematically irrecoverable. Exceeds Art. 17 requirements.',
      },
      {
      ...
      },
      {
        ...
        mitchImplementation:
          'Per-verifier consent revocation. Remembered approvals can be revoked anytime.',
      },
      {
        ...
        mitchImplementation:
          'Every request shows equal-weight Approve/Decline. Verifier blocking available.',
      },
    ],

    technicalSafeguards: [
      {
        measure: 'Selective disclosure (SD-JWT)',
        description: 'Share only the specific claims needed. Verifier never sees other data.',
      },
      {
        measure: 'Crypto-shredding',
        description:
          'Raw data encrypted with ephemeral keys. Keys destroyed after use. Data irrecoverable.',
      },
      {
        measure: 'Response padding',
        description:
          "All responses padded to fixed 4KB. Network observers can't infer content from size.",
      },
      {
        measure: 'Timing jitter',
        description: 'Random 50-200ms delay on all responses. Prevents timing correlation.',
      },
      {
        measure: 'Per-session identifiers',
        description:
          "Each presentation uses a unique derived ID. Verifiers can't correlate across sessions.",
      },
      {
        measure: 'StatusList2021 revocation',
        description:
          "Revocation via public bitstring. Issuer can't tell which credential was checked.",
      },
      {
        measure: 'Identical decline responses',
        description:
          "Declined and missing credentials produce identical responses. Verifier can't distinguish.",
      },
      {
        measure: 'Tamper-evident audit chain',
        description:
          'All actions logged in hash-chain. Tampering breaks the chain and is detectable.',
      },
      {
        measure: 'No server-side PII storage',
        description:
          "miTch infrastructure stores zero personal data. All PII stays on user's device.",
      },
    ],
  };
}
