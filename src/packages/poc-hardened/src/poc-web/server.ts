...
function handleIssue(): object {
  ...
  const rawPII = JSON.stringify({
    name: 'Jonas',
    birthdate: '1998-05-12',
    email: 'jonas@example.at',
  });
...
function handlePresent(body: { disclose: string[] }): object {
  ...
  const revoked =
    credential.meta.statusListIndex !== undefined
  ...
  const consentAction =
    disclose.length === allClaims.length
      ? ('approved' as const)
      : disclose.length === 0
        ? ('declined' as const)
        : ('partial' as const);
  ...
  const claimRecords: ConsentClaimRecord[] = allClaims.map((name) => ({
    name,
    tier:
      name === 'over_18' || name === 'over_16' || name === 'over_21'
        ? ('legal' as const)
        : ('optional' as const),
  ...
  // Audit trail
  const auditAction =
    verification.valid && !revoked
      ? ('verification_allowed' as const)
      : ('verification_denied' as const);
