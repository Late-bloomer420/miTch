...
export class VerifierClient {
  ...
  public async verifyResponse(
    ...
    const cekBuffer = await window.crypto.subtle.decrypt({ name: 'RSA-OAEP' }, privKey, encKey);

    // 4. Import CEK (AES-GCM)
    const cek = await window.crypto.subtle.importKey('raw', cekBuffer, { name: 'AES-GCM' }, false, [
      'decrypt',
    ]);
