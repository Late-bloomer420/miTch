...
export class BrowserVerifier {
  ...
  constructor(config: BrowserVerifierConfig, sessionStorage?: SessionStorage) {
    // Apply defaults
    this.config = {
      ...config,
   ...
   */
  async verifyResponse(response: WalletResponse): Promise<VerifiedResponse> {
    // 1. Retrieve session
   ...
   */
  async waitForResponse(sessionId: string, timeoutMs: number = 60_000): Promise<VerifiedResponse> {
