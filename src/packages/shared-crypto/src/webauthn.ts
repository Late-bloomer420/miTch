...
import { ASKMI_STORAGE_KEYS } from '@askmi/shared-types';
...
const PASSKEY_DB_NAME = ASKMI_STORAGE_KEYS.passkeyDb;
const PASSKEY_STORE_NAME = 'passkeys';
const PASSKEY_STORAGE_KEY = ASKMI_STORAGE_KEYS.passkeyRegistration;
const IDENTITY_KEY_STORAGE_KEY = ASKMI_STORAGE_KEYS.identityKeyRegistration;
...
export class WebAuthnService {
  ...
  static async signWithIdentityKey(data: string): Promise<string> {
    ...
    const challenge =
      dataBuffer.length > 64 ? await crypto.subtle.digest('SHA-256', dataBuffer) : dataBuffer;
  ...
  static async provePresenceDetailed(
    ...
    if (timeoutMinutes > 0) {
      const cachedSessionRaw = sessionStorage.getItem(ASKMI_STORAGE_KEYS.webauthnSession);
      if (cachedSessionRaw) {
        try {
          ...
          if (ageMinutes <= timeoutMinutes) {
            ...
            sessionStorage.removeItem(ASKMI_STORAGE_KEYS.webauthnSession);
          }
        } catch (_e) {
          // invalid cache
          sessionStorage.removeItem(ASKMI_STORAGE_KEYS.webauthnSession);
    ...
    if (timeoutMinutes > 0) {
      sessionStorage.setItem(
        ASKMI_STORAGE_KEYS.webauthnSession,
        JSON.stringify({
          ...proof,
          timestamp: Date.now(),
        })
