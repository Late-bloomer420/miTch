...
export async function assertCryptoCapabilities(): Promise<void> {
  ...
  await crypto.subtle
    ...
    .catch(() => {
      throw new Error('CRYPTO_UNAVAILABLE: AES-GCM-256 not supported on this platform');
    });
  ...
  await crypto.subtle
    ...
    .catch(() => {
      throw new Error('CRYPTO_UNAVAILABLE: ECDSA P-256 not supported on this platform');
    });

  // HKDF — pairwise DID key derivation
  const hkdfBase = await crypto.subtle
    .importKey('raw', new Uint8Array(32), { name: 'HKDF' }, false, ['deriveBits'])
    .catch(() => {
      throw new Error('CRYPTO_UNAVAILABLE: HKDF not supported on this platform');
    });
  ...
  await crypto.subtle
    ...
    .catch(() => {
      throw new Error('CRYPTO_UNAVAILABLE: HKDF deriveBits failed on this platform');
    });
}
