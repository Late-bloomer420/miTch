...
export class DIDResolver {
  ...
  async extractVerificationKey(
    ...
    const purposeRefs =
      (purpose === 'authentication' ? doc.authentication : doc.assertionMethod) ?? [];
    ...
    if (purposeRefs.length > 0) {
      ...
      const found = methods.find(
        (m) => purposeRefs.includes(m.id) || purposeRefs.includes(m.id.split('#').pop()!)
      );
 ...
 */
export function detectKeyAlgorithm(
  jwk: JsonWebKey
