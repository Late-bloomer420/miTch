...
describe('SD-JWT VC Issuance', () => {
  ...
  it('throws if iss is missing', async () => {
    ...
    ).rejects.toThrow('iss is required');
  });
  ...
  it('throws if vct is not a URI', async () => {
    ...
    ).rejects.toThrow('vct must be a URI');
  });
...
describe('Key Binding JWT (kb+jwt)', () => {
  const SD_JWT_WITH_DISCLOSURES =
    'eyJhbGciOiJFUzI1NiIsInR5cCI6InZjK3NkLWp3dCJ9.payload.sig~disclosure1~disclosure2~';
