...
describe('didWebToUrl', () => {
  ...
  it('converts localhost with percent-encoded port', () => {
    expect(didWebToUrl('did:web:localhost%3A3002')).toBe(
      'http://localhost:3002/.well-known/did.json'
    );
  });

  it('converts localhost with port and path', () => {
    expect(didWebToUrl('did:web:localhost%3A3002:user:bob')).toBe(
      'http://localhost:3002/user/bob/did.json'
    );
  });
...
describe('DIDResolver', () => {
  ...
  it('DENY on HTTP error (fail-closed)', async () => {
    ...
    await expect(resolver.resolve('did:web:example.com')).rejects.toThrow(DIDResolutionError);
  });
  ...
  it('DENY on malformed DID document (missing id)', async () => {
    ...
    await expect(resolver.resolve('did:web:example.com')).rejects.toThrow(/missing "id" field/);
  });
  ...
  it('DENY on malformed DID document (missing @context)', async () => {
    ...
    await expect(resolver.resolve('did:web:example.com')).rejects.toThrow(/@context/);
  });

  it('DENY on unsupported DID method (no mock fallback)', async () => {
    await expect(resolver.resolve('did:key:z6Mk...')).rejects.toThrow(/Unsupported DID method/);
  });
  ...
  it('DENY on timeout (fail-closed)', async () => {
    ...
    await expect(resolver.resolve('did:web:slow.example.com')).rejects.toThrow(DIDResolutionError);
  });
});
...
describe('DIDResolver.extractVerificationKey', () => {
  ...
  it('DENY when no verificationMethod', async () => {
    ...
    await expect(resolver.extractVerificationKey(doc)).rejects.toThrow(DIDKeyExtractionError);
  });
  ...
  it('DENY when no publicKeyJwk', async () => {
    ...
    await expect(resolver.extractVerificationKey(doc)).rejects.toThrow(/no publicKeyJwk/);
  });
});
...
describe('P0 hardening findings', () => {
  it('DENY: did:web localhost is blocked by default', async () => {
    ...
    await expect(resolver.resolve('did:web:localhost%3A3002')).rejects.toThrow(
  ...
  it('DENY: legacy resolveDID no longer allows mock fallback', async () => {
    await expect(resolveDID('did:key:z6MklegacyFallbackAttempt')).rejects.toThrow(
