...
export async function demonstrateSecurePresentation() {
  ...
  const userKey = await UserDerivedKeyProtection.deriveKeyFromUser(
  ...
  console.log(
    `📦 GDPR Export: ${exportData.entries.length} entries. Root Hash: ${exportData.integrityProof.rootHash}`
  );
