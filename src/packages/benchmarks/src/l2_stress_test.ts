...
async function runStressTest() {
  ...
  log.setAuditKeys(keys.privateKey, keys.publicKey, 'audit-key-2026-v1');
  console.log(
    `[Setup] Environment initialized in ${(performance.now() - setupStart).toFixed(2)}ms`
  );
  ...
  console.log(
    `Status:              ${anchorTime < 1000 ? '✅ EXCELLENT (<1s Anchor)' : '⚠️ ATTENTION (>1s Anchor)'}`
  );
