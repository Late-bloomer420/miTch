...
async function benchmarkUnlock(): Promise<number> {
  ...
  const baseKey = await crypto.subtle.importKey('raw', password, { name: 'PBKDF2' }, false, [
    'deriveKey',
  ]);
...
function benchmarkPolicyEval(ruleCount: number): number {
  ...
  if (matchedRule) {
    // Claim Intersection
    const _effectiveClaims = requestedClaims.filter(
      (c) => matchedRule.allowedClaims.includes(c) && !matchedRule.deniedClaims.includes(c)
    );
  }
...
async function benchmarkSigning(): Promise<number> {
  ...
  const data = new TextEncoder().encode(
    JSON.stringify({ decision_id: 'test-123', verdict: 'ALLOW' })
  );
...
async function runBenchmarks() {
  ...
  console.log(
    `   Result: ${unlockResult.duration_ms}ms [${unlockResult.status}] (Budget: ${unlockResult.budget_ms}ms)`
  );
  ...
  console.log(
    `   Result: ${policyResult100.duration_ms}ms [${policyResult100.status}] (Budget: ${policyResult100.budget_ms}ms)`
  );
  ...
  console.log(
    `   Result: ${policyResult500.duration_ms}ms [${policyResult500.status}] (Budget: ${policyResult500.budget_ms}ms)`
  );
  ...
  console.log(
    `   Result: ${encryptResult.duration_ms}ms [${encryptResult.status}] (Budget: ${encryptResult.budget_ms}ms)`
  );
  ...
  console.log(
    `   Result: ${wrapResult.duration_ms}ms [${wrapResult.status}] (Budget: ${wrapResult.budget_ms}ms)`
  );
  ...
  console.log(
    `   Result: ${signResult.duration_ms}ms [${signResult.status}] (Budget: ${signResult.budget_ms}ms)`
  );
  ...
  results.forEach((r) => {
    ...
    console.log(
      `  ${icon} ${r.name.padEnd(30)} ${r.duration_ms.toString().padStart(8)}ms / ${r.budget_ms}ms`
    );
  });
  ...
  console.log(
    `  Status:              ${failCount === 0 ? (warnCount === 0 ? '✅ ALL PASS' : '⚠️ WARNINGS') : '❌ FAILURES'}`
  );
  console.log('');
