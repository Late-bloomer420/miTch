 ...
 */
export function getInclusionProof(tree: MerkleBuildResult, targetHash: Hex32): InclusionProof {
  ...
  for (let i = 0; i < tree.levels.length - 1; i++) {
    ...
    const pairNode = pairIndex < level.length ? level[pairIndex] : level[currentIndex];
