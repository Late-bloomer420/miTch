...
export class EphemeralKey implements IEphemeralKey {
  ...
  static generate(lengthBytes: number = 32): EphemeralKey {
    const randomKey = webcrypto.getRandomValues(new Uint8Array(lengthBytes));
   ...
   */
  async use<T>(operation: (key: Uint8Array) => Promise<T>, timeoutMs: number = 30000): Promise<T> {
  ...
  private async executeOperation<T>(operation: (key: Uint8Array) => Promise<T>): Promise<T> {
