...
describe('shared-types/predicates canonicalization', () => {
  it('canonicalizePredicate is stable for semantically identical predicates', () => {
    const p1: Predicate = {
      ...
      expression: {
        logic: 'and',
        clauses: [{ path: 'credentialSubject.birthDate', op: 'gte', value: 18, type: 'age_years' }],
      },
    };
    ...
    const p2: Predicate = {
      ...
      expression: {
        logic: 'and',
        clauses: [{ op: 'gte', path: 'credentialSubject.birthDate', type: 'age_years', value: 18 }],
      },
    };
