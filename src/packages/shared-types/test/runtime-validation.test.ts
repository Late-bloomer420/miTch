...
import { quantizeTimeSlot, quantizeBudget, quantizeDayBucket } from '../src/ad-preferences.js';
...
describe('calculateTrustScore — deny-listed', () => {
  ...
  it('deny-listed overrides high transaction count', () => {
    expect(
      calculateTrustScore(makeRep({ denyListed: true, successfulTransactions: 1_000_000 }))
    ).toBe(0);
  });
});
...
describe('calculateTrustScore — base (no transactions, no violations)', () => {
  it('returns 50 for a brand-new verifier', () => {
    const score = calculateTrustScore(
      makeRep({ successfulTransactions: 0, confirmedViolations: 0 })
    );
...
describe('calculateTrustScore — violation penalty', () => {
  it('violations reduce score', () => {
    const clean = calculateTrustScore(
      makeRep({ successfulTransactions: 100, confirmedViolations: 0 })
    );
    const dirty = calculateTrustScore(
      makeRep({ successfulTransactions: 100, confirmedViolations: 1 })
    );
  ...
  it('high violation rate pushes score toward 0', () => {
    ...
    const score = calculateTrustScore(
      makeRep({ successfulTransactions: 10, confirmedViolations: 10 })
    );
...
describe('quantizeTimeSlot', () => {
  ...
  it('hour 14 → SLOT_AFTERNOON', () =>
    expect(quantizeTimeSlot(makeDate(14))).toBe('SLOT_AFTERNOON'));
  it('hour 17 → SLOT_AFTERNOON', () =>
    expect(quantizeTimeSlot(makeDate(17))).toBe('SLOT_AFTERNOON'));
  ...
  it('covers all 24 hours with exactly 6 distinct buckets', () => {
    const buckets = new Set(Array.from({ length: 24 }, (_, h) => quantizeTimeSlot(makeDate(h))));
