...
**Risk**: A malicious Agent requests a VP for "Lufthansa" but provides its _own_ Public Key. If the wallet accepts this, the Agent can decrypt the data.
...
3.  **UI Warning**: If the key cannot be verified, the UI must warn: _"The target Identity cannot be verified. This might be a fake airline."_
