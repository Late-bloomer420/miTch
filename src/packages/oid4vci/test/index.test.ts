...
describe('OID4VCIIssuer (Privacy Firewall)', () => {
  ...
  it('should FAIL_CLOSED on invalid schema input', async () => {
    ...
    await expect(issuer.issueCredential(invalidRequest)).rejects.toThrow(/FAIL_INPUT_ARBITRATION/);
  });
  ...
  it('should not leak PII in error messages', async () => {
    ...
    await expect(issuer.issueCredential(requestWithBadDate)).rejects.toThrow();
