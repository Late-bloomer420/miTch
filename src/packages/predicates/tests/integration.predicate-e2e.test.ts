...
describe('Predicate E2E: Wallet → Verifier', () => {
  ...
  describe('Wallet: evaluatePredicates()', () => {
    it('evaluates age predicate and returns result without PII', async () => {
      ...
      const result = await evaluatePredicates(mockCredential, request, mockWalletSign);
    ...
    it('evaluates multiple predicates (AND semantics)', async () => {
      const request: PredicateRequest = {
        ...
        predicates: [CommonPredicates.ageAtLeast(18), CommonPredicates.euResident()],
        timestamp: new Date().toISOString(),
      };
    ...
    it('throws on missing nonce (security violation)', async () => {
      ...
      await expect(evaluatePredicates(mockCredential, request, mockWalletSign)).rejects.toThrow(
        'SECURITY_VIOLATION'
      );
    });
  ...
  describe('Verifier: verifyPredicateResult()', () => {
    it('validates a legitimate predicate result', async () => {
      ...
      const walletResult = await evaluatePredicates(mockCredential, request, mockWalletSign);

      const allowedHashes = await buildAllowedPredicateSet([CommonPredicates.ageAtLeast(18)]);
    ...
    it('detects unknown predicate hash (policy enforcement)', async () => {
      ...
      const allowedHashes = await buildAllowedPredicateSet([CommonPredicates.ageAtLeast(21)]);
  ...
  describe('Canonicalization: Deterministic hashing', () => {
    ...
    it('produces identical hashes for requests with different predicate order', async () => {
      ...
      const request2: PredicateRequest = {
        ...
        nonce: 'nonce-test',
        purpose: 'Test',
        predicates: [CommonPredicates.euResident(), CommonPredicates.ageAtLeast(18)],
        timestamp: '2025-01-28T10:00:00Z',
      };
