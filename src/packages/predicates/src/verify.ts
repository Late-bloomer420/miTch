...
import { PredicateResult, PredicateVerificationResult } from './types';
...
export async function verifyPredicateResult(
  ...
  if (proof.binding.verifierDid !== originalRequest.verifierDid) {
    errors.push(
      `Verifier DID mismatch: expected ${originalRequest.verifierDid}, got ${proof.binding.verifierDid}`
    );
  }
  ...
  if (proof.binding.requestHash !== expectedRequestHash) {
    errors.push(
      `Request hash mismatch: expected ${expectedRequestHash}, got ${proof.binding.requestHash}`
    );
  }
  ...
  if (isNaN(evaluatedAt)) {
    ...
    if (Math.abs(age) > maxSkewMs) {
      errors.push(
        `Signature expired or clock skew too large (diff: ${age}ms, max: ${maxSkewMs}ms)`
      );
    }
  }
