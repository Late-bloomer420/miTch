...
describe('generateNullifier', () => {
  it('produces deterministic output for same input', () => {
    const seed = makeSeed();
    const r1 = generateNullifier({
      userSeed: seed,
      verifierDid: VERIFIER_A,
      scopeId: SCOPE_CAMPAIGN_1,
    });
    const r2 = generateNullifier({
      userSeed: seed,
      verifierDid: VERIFIER_A,
      scopeId: SCOPE_CAMPAIGN_1,
    });
  ...
  it('produces different nullifiers for different user seeds', () => {
    const r1 = generateNullifier({
      userSeed: makeSeed(0x11),
      verifierDid: VERIFIER_A,
      scopeId: SCOPE_CAMPAIGN_1,
    });
    const r2 = generateNullifier({
      userSeed: makeSeed(0x22),
      verifierDid: VERIFIER_A,
      scopeId: SCOPE_CAMPAIGN_1,
    });
    expect(r1.nullifier).not.toBe(r2.nullifier);
...
describe('verifyNullifierScope', () => {
  ...
  it('rejects a tampered scope binding', () => {
    ...
    const result = verifyNullifierScope(
      nullifier,
      VERIFIER_A,
      SCOPE_CAMPAIGN_1,
...
describe('Cross-verifier isolation', () => {
  it('verifier A cannot use verifier B nullifier for same scope', () => {
    ...
    const forA = generateNullifier({
      userSeed: seed,
      verifierDid: VERIFIER_A,
      scopeId: SCOPE_CAMPAIGN_1,
    });
    const forB = generateNullifier({
      userSeed: seed,
      verifierDid: VERIFIER_B,
      scopeId: SCOPE_CAMPAIGN_1,
    });
    ...
    const crossCheck = verifyNullifierScope(
      forA.nullifier,
      VERIFIER_B,
      SCOPE_CAMPAIGN_1,
      forA.scopeBinding
    ...
    expect(
      verifyNullifierScope(forA.nullifier, VERIFIER_A, SCOPE_CAMPAIGN_1, forA.scopeBinding).valid
    ).toBe(true);
    expect(
      verifyNullifierScope(forB.nullifier, VERIFIER_B, SCOPE_CAMPAIGN_1, forB.scopeBinding).valid
    ).toBe(true);
  });
});
