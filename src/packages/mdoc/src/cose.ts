...
coseTags[CBOR_TAGS.EMBEDDED_CBOR] = (innerBytes: any) =>
  cborgDecode(innerBytes, COSE_DECODE_OPTIONS);
...
function buildSigStructure1(
  ...
  const structure = ['Signature1', protectedHeaderBytes, externalAad, payload];
...
export async function createSign1(opts: Sign1CreateOptions): Promise<Uint8Array> {
  ...
  const coseArray = [protectedHeaderBytes, unprotected, payload, signature];
...
export function decodeCoseSign1(coseSign1Bytes: Uint8Array): CoseSign1Structure {
  ...
  const protectedMap: CoseHeaderMap =
  ...
  const unprotectedHeaders: CoseHeaderMap =
    unprotectedRaw instanceof Map ? unprotectedRaw : new Map<number, unknown>();

  // Payload: bstr or null
  const payload = payloadRaw instanceof Uint8Array ? payloadRaw : payloadRaw === null ? null : null;
...
export async function createMac0(opts: Mac0CreateOptions): Promise<Uint8Array> {
  ...
  const tagBuffer = await crypto.subtle.sign('HMAC', macKey, toArrayBuffer(macStructure));
