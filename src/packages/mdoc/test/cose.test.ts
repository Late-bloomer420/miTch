...
beforeAll(async () => {
  ...
  ecdhKeyPairA = await crypto.subtle.generateKey({ name: 'ECDH', namedCurve: 'P-256' }, false, [
    'deriveBits',
    'deriveKey',
  ]);
  ecdhKeyPairB = await crypto.subtle.generateKey({ name: 'ECDH', namedCurve: 'P-256' }, false, [
    'deriveBits',
    'deriveKey',
  ]);
});
...
describe('COSE_Sign1 headers', () => {
  ...
  test('unprotected headers are preserved', async () => {
    ...
    const kid = parsed.unprotectedHeaders.get(COSE_HEADER.KID) ?? parsed.unprotectedHeaders.get(4);
...
describe('COSE_Mac0 decode (structural)', () => {
  ...
  test('decodeCoseMac0 rejects wrong-length array', () => {
    expect(() => decodeCoseMac0(encode([new Uint8Array(0), {}]))).toThrow(
