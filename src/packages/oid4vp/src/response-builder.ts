...
export function buildAuthorizationResponse(
  ...
  const token =
    typeof vpToken.vp_token === 'string' ? vpToken.vp_token : (vpToken.vp_token as string[])[0];
...
export function encodeDirectPost(response: AuthorizationResponse): string {
  ...
  const token =
    typeof response.vp_token === 'string' ? response.vp_token : JSON.stringify(response.vp_token);
