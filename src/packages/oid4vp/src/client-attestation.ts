...
export async function validateClientAttestationChain(
  ...
  try {
    const providerKey =
      opts.providerPublicKey instanceof CryptoKey
    ...
    return {
      ok: false,
      errors: [
        `Client Attestation verification failed: ${e instanceof Error ? e.message : String(e)}`,
      ],
    };
  }
  ...
  if (attestationPayload.exp < now) {
    errors.push(
      `Client Attestation expired at ${new Date(attestationPayload.exp * 1000).toISOString()}`
    );
  }
  ...
  try {
    ...
    return {
      ok: false,
      errors: [
        `Client Attestation PoP verification failed: ${e instanceof Error ? e.message : String(e)}`,
      ],
    };
  }
