...
export async function validateVerifierAttestation(
  ...
  try {
    const key =
      trustAnchorPublicKey instanceof CryptoKey
    ...
    return {
      ok: false,
      errors: [
        `Verifier Attestation verification failed: ${e instanceof Error ? e.message : String(e)}`,
      ],
    };
  }
  ...
  if (opts?.expectedClientId && payload.sub !== opts.expectedClientId) {
    errors.push(
      `Verifier client_id mismatch: expected ${opts.expectedClientId}, got ${payload.sub}`
    );
  }
...
export async function validateHAIPRequest(
  ...
  if (r['client_id_scheme'] !== 'verifier_attestation') {
    errors.push(
      `HAIP requires client_id_scheme=verifier_attestation, got: ${r['client_id_scheme']}`
    );
  }
...
export async function encryptDirectPostResponse(
  ...
  const recipientKey =
    verifierPublicKey instanceof CryptoKey
      ? verifierPublicKey
