...
export function parseAuthorizationRequest(raw: unknown): ValidationResult<AuthorizationRequest> {
  ...
  return {
    ...
    value: {
      ...
      response_mode:
        typeof r['response_mode'] === 'string'
          ? (r['response_mode'] as AuthorizationRequest['response_mode'])
          : 'direct_post',
      client_metadata:
        r['client_metadata'] && typeof r['client_metadata'] === 'object'
 ...
 */
export function parsePresentationDefinition(
  raw: unknown
): ValidationResult<PresentationDefinition> {
  if (!raw || typeof raw !== 'object') {
    return {
      ok: false,
      error: 'presentation_definition must be an object',
      code: 'MISSING_PRESENTATION_DEFINITION',
    };
  }
  ...
  if (!Array.isArray(pd['input_descriptors']) || pd['input_descriptors'].length === 0) {
    return {
      ok: false,
      error: 'input_descriptors must be non-empty array',
      code: 'MISSING_INPUT_DESCRIPTORS',
    };
  }
  ...
  for (const item of pd['input_descriptors']) {
    ...
    if (typeof d['id'] !== 'string' || !d['id']) {
      return {
        ok: false,
        error: 'Each input_descriptor must have an id',
...
export function requiresSelectiveDisclosure(pd: PresentationDefinition): boolean {
  return pd.input_descriptors.some((d) => d.constraints?.limit_disclosure === 'required');
}
