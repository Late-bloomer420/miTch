...
describe('buildOID4VPRequest', () => {
  const defaultOpts = {
    verifierClientId: 'did:askmi:verifier',
  ...
  it('successfully builds a request for a valid scenario', () => {
    ...
    expect(result.nonce).toHaveLength(32); // randomHex(16) -> 32 hex chars
    ...
    expect(result.request.state).toHaveLength(16); // randomHex(8) -> 16 hex chars
    expect(result.request.presentation_definition).toEqual(
      SCENARIO_PRESENTATION_DEFINITIONS['liquor-store']
    );
  ...
  it('works for all defined scenarios', () => {
    for (const scenarioId of Object.keys(SCENARIO_PRESENTATION_DEFINITIONS)) {
      ...
      expect(result.request.presentation_definition).toEqual(
        SCENARIO_PRESENTATION_DEFINITIONS[scenarioId]
      );
    }
  });
});
