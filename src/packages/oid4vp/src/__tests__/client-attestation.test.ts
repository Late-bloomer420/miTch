...
beforeAll(async () => {
  const provPair = await crypto.subtle.generateKey({ name: 'ECDSA', namedCurve: 'P-256' }, true, [
    'sign',
    'verify',
  ]);
  ...
  const walletPair = await crypto.subtle.generateKey({ name: 'ECDSA', namedCurve: 'P-256' }, true, [
    'sign',
    'verify',
  ]);
...
async function buildValidChain(overrides?: { nonce?: string; audience?: string }) {
...
describe('Client Attestation Chain Validation', () => {
  ...
  it('rejects attestation signed by unknown provider', async () => {
    ...
    expect(result.ok).toBe(false);
    expect(result.errors.some((e) => e.includes('Client Attestation verification failed'))).toBe(
      true
    );
  });
