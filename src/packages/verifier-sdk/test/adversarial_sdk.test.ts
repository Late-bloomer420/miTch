...
describe('Adversarial Tests: Verifier SDK Binding (V1-V3)', () => {
  ...
  test('V1: Replay Attack - pkg for Verifier A is rejected by Verifier B (AAD Binding)', async () => {
    ...
    const proofPublicJWK = await (globalThis as any).crypto.subtle.exportKey(
      'jwk',
      proofKeys.publicKey
    );
    ...
    const artifact = {
      vp,
      proof: { alg: 'ES256', signature: signatureHex, public_key: proofPublicJWK },
    };

    // Bind to DID_A
    const aadForA = new TextEncoder().encode(
      canonicalStringify({
    ...
    await expect(sdkB.verifyPresentation(JSON.stringify(pkgForA))).rejects.toThrow(
      /SECURITY_VIOLATION|Key Unwrap Failed/i
    );
  });
  ...
  test('V2: Recipient Binding Manipulation - changing verifier_did in context is detected', async () => {
    ...
    const aad = new TextEncoder().encode(
      canonicalStringify({ decision_id: 'd1', nonce: 'n1', verifier_did: DID_A })
    );
    const ciphertext = await ephemeralKey.encrypt(
      JSON.stringify({ vp: { metadata: { nonce: 'n1' } }, proof: {} }),
      aad
    );
    const encryptedKey = await ephemeralKey.sealToRecipient(verifierKeysA.publicKey);
    ...
    await expect(sdkA.verifyPresentation(JSON.stringify(pkg))).rejects.toThrow(
      /addressed to .* verifier|AAD Binding Violation/i
    );
  });
  ...
  test('V3: Integrity Tamper - modifying ciphertext results in decryption failure', async () => {
    ...
    const tamperedCiphertext =
      pkg.ciphertext.substring(0, 10) +
      (pkg.ciphertext[10] === 'A' ? 'B' : 'A') +
      pkg.ciphertext.substring(11);
    pkg.ciphertext = tamperedCiphertext;

    await expect(sdkA.verifyPresentation(JSON.stringify(pkg))).rejects.toThrow(); // AES-GCM tag mismatch
  });
});
