...
describe('VerifierSDK Attack Vectors', () => {
  ...
  it('should REJECT incorrect Verifier DID (Address Check)', async () => {
    ...
    await expect(verifierSDK.verifyPresentation(hijackedPkg)).rejects.toThrow(AADValidationError);
  });

  it('should REJECT tampering with Ciphertext', async () => {
    const pkg = await createValidPackage({ tamperedCiphertext: true });
    await expect(verifierSDK.verifyPresentation(pkg)).rejects.toThrow(DecryptError);
  });
  ...
  it('should REJECT tampering with AAD (Context)', async () => {
    ...
    await expect(verifierSDK.verifyPresentation(JSON.stringify(pkg))).rejects.toThrow(DecryptError);
  });
  ...
  it('should REJECT Replay Attacks', async () => {
    ...
    await expect(verifierSDK.verifyPresentation(pkg)).rejects.toThrow(ReplayDetectedError);
  });
  ...
  it('should REJECT Invalid Proof Signature', async () => {
    ...
    const tamperedCiphertext = await ephemeralKey.encrypt(
    ...
    await expect(verifierSDK.verifyPresentation(JSON.stringify(tamperedPkg))).rejects.toThrow(
      ProofSignatureError
    );
  });
});
