...
export class TransportParseError extends VerifierError {
  constructor(details: string) {
    super(`Transport Package Invalid: ${details}`, 'TRANSPORT_PARSE_ERROR');
  }
}

export class KeyUnwrapError extends VerifierError {
  constructor(details: string) {
    super(`Key Unwrap Failed: ${details}`, 'KEY_UNWRAP_ERROR');
  }
}

export class DecryptError extends VerifierError {
  constructor(details: string) {
    super(`Decryption Failed (AEAD): ${details}`, 'DECRYPT_ERROR');
  }
}

export class AADValidationError extends VerifierError {
  constructor(details: string) {
    super(`AAD Binding Violation: ${details}`, 'AAD_VALIDATION_ERROR');
  }
}

export class ProofSignatureError extends VerifierError {
  constructor(details: string) {
    super(`Proof Signature Invalid: ${details}`, 'PROOF_SIGNATURE_ERROR');
  }
}

export class ReplayDetectedError extends VerifierError {
  constructor(nonce: string) {
    super(`Replay Detected: Nonce ${nonce} already seen`, 'REPLAY_DETECTED');
  }
}

export class TokenExpiredError extends VerifierError {
  constructor(ageMs: number) {
    super(`Token Expired: Age ${ageMs}ms > TTL`, 'TOKEN_EXPIRED');
  }
}
