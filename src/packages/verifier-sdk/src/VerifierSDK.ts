import { decrypt, canonicalStringify } from '@mitch/shared-crypto';
...
import { VerifierRequest } from '@mitch/shared-types';
import {
  TransportPackage,
  VerificationResult,
  TransportParseError,
...
export class VerifierSDK {
   ...
   */
  async verifyPresentation<T = unknown>(
    input: string | TransportPackage
    ...
    try {
      ...
      throw new KeyUnwrapError(
        e instanceof Error ? e.message : 'Check Private Key and Recipient Key Format'
      );
    }

    // 3. AAD Binding Check (Address Verification)
    if (aad_context.verifier_did !== this.config.verifierDid) {
      throw new AADValidationError(
        `Package addressed to ${aad_context.verifier_did}, expected ${this.config.verifierDid}`
      );
    }
