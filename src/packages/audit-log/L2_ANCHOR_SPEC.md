...
4.  **Local Binding**: These values are stored in a new local table `anchors` and appended to the _next_ Audit Log entry as metadata, creating a "Back-Link" from the chain.
...
- **Rollback Protection**: If an attacker reverts the wallet to yesterday's state, the DPA will find that the _latest_ anchor on-chain does not match the provided local history.
