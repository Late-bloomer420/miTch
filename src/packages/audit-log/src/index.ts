...
export class AuditLog {
  ...
  async append(
    ...
    if (!this.kid) {
      throw new Error(
        'AUDIT_LOG_UNINITIALIZED: setAuditKeys must be called with a valid kid before appending entries.'
      );
    }

    const previousHash =
      this.entries.length > 0 ? this.entries[this.entries.length - 1].currentHash : '0'.repeat(64);
  ...
  async syncToL2(): Promise<L2AnchorReceipt> {
    ...
    const entriesCanonical = canonicalStringify(
      this.entries.map((e) => ({
        id: e.id,
        hash: e.currentHash,
        sig: e.signature,
  ...
  getShreddingCertificateByDecision(decisionId: string): AuditLogEntry[] {
    return this.entries.filter(
      (e) => e.action === 'KEY_DESTROYED' && e.metadata?.decision_id === decisionId
    );
  }
