...
export class EscrowBackupService {
   ...
   */
  async createBackup(entries: AuditLogEntry[], walletId: string): Promise<EncryptedBackup> {
    if (entries.length === 0) {
   ...
   */
  async decryptBackup(backup: EncryptedBackup, dpaPrivateKey: CryptoKey): Promise<AuditLogEntry[]> {
