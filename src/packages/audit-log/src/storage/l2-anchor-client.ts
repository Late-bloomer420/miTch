...
export class L2AnchorClient {
   ...
   */
  async anchorRoot(stateRoot: string, options?: Partial<L2AnchorConfig>): Promise<L2AnchorReceipt> {
  ...
  private async submitToL2(
    ...
    console.warn(
      '[L2AnchorClient] Real blockchain anchoring not yet wired — returning mock receipt'
    );
  ...
  private async queueForBatch(stateRoot: string): Promise<L2AnchorReceipt> {
    ...
    console.info(
      `[L2AnchorClient] Queued for batch anchoring (${this.pendingBatch.length} pending)`
    );
  ...
  private getDefaultContractAddress(network: L2Network): string {
    const envMap: Record<L2Network, string> = {
      'optimism-mainnet':
        process.env['L2_CONTRACT_OPTIMISM_MAINNET'] ?? '0x0000000000000000000000000000000000000000',
      'optimism-sepolia':
        process.env['L2_CONTRACT_OPTIMISM_SEPOLIA'] ?? '0x0000000000000000000000000000000000000000',
      'arbitrum-one':
        process.env['L2_CONTRACT_ARBITRUM_ONE'] ?? '0x0000000000000000000000000000000000000000',
      'arbitrum-sepolia':
        process.env['L2_CONTRACT_ARBITRUM_SEPOLIA'] ?? '0x0000000000000000000000000000000000000000',
      mock: '',
    };
  ...
  async verifyAnchor(receipt: L2AnchorReceipt): Promise<boolean> {
    ...
    if (!contractAddress || contractAddress === '0x0000000000000000000000000000000000000000') {
      ...
      console.warn(
        `[L2AnchorClient] verifyAnchor: contract not deployed for ${receipt.network}, skipping on-chain check`
      );
      return true;
    }
    ...
    console.warn(
      `[L2AnchorClient] verifyAnchor: on-chain query stub for ${receipt.network} tx=${receipt.l2TransactionId}`
    );
    return true;
  }
}
