...
export class IndexedDBAuditStore {
  ...
  async getAllEntries(): Promise<AuditLogEntry[]> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        const entries = request.result as (AuditLogEntry & {
          walletId: string;
          storedAt: string;
        })[];
        ...
        const cleanEntries = entries.map(
          ({ walletId: _walletId, storedAt: _storedAt, ...entry }) => entry
        );
  ...
  async getEntriesByTimeRange(startTime: string, endTime: string): Promise<AuditLogEntry[]> {
    ...
    return allEntries.filter((entry) => entry.timestamp >= startTime && entry.timestamp <= endTime);
  }
  ...
  async getEntriesByAction(action: AuditLogEntry['action']): Promise<AuditLogEntry[]> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        // Remove internal fields
        const cleanEntries = walletEntries.map(
          ({ walletId: _walletId, storedAt: _storedAt, ...entry }) => entry
        );
        resolve(cleanEntries);
      };
