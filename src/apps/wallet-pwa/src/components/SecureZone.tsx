 ...
 */
export const SecureZone: React.FC<SecureZoneProps> = ({
  children,
  onIntervention,
  className,
  style: externalStyle,
  ...
  const checkOverlay = () => {
    ...
    const isSelfOrChild =
      containerRef.current === topElement || containerRef.current.contains(topElement);
  ...
  const handleCapture = (e: React.MouseEvent) => {
    ...
    const isSelfOrChild =
      containerRef.current === topElement || containerRef.current.contains(topElement as Node);
  ...
  return (
    <div
      ...
      style={
        !isSecure
          ? { ...externalStyle, border: '2px solid red', pointerEvents: 'none', opacity: 0.5 }
          : { ...externalStyle }
      }
    >
