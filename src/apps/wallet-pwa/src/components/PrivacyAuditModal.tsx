...
export const PrivacyAuditModal: React.FC<PrivacyAuditModalProps> = ({
  ...
  if (error) {
    return (
      ...
      >
        <div
          style={{ background: '#111', padding: 20, borderRadius: 12, border: '1px solid #333' }}
        >
          <h3 style={{ color: '#ef4444' }}>Audit Failed</h3>
          <p style={{ color: '#999' }}>{error}</p>
          <button onClick={runAudit} style={{ padding: '8px 16px', marginTop: 10 }}>
            Retry
          </button>
          <button onClick={onCancel} style={{ padding: '8px 16px', marginLeft: 10 }}>
            Cancel
          </button>
        </div>
      </div>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', opacity: 0.8 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: '11px', color: '#9ca3af' }}>
                  Sees metadata only.{' '}
                  <span style={{ color: '#10b981' }}>Crypto-Shredding active.</span>
                </div>
              </div>
            </div>
          </div>
