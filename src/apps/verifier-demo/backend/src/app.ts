...
app.get('/', (req, res) => {
  res
    .type('text/plain')
    .send('miTch Verifier Backend OK. Try /status or open the verifier frontend.');
});
...
app.post('/wallet-present', async (req, res) => {
  ...
  try {
    ...
    if (validation.ok) {
      ...
      return res
        .status(403)
        .json({ ok: false, errors: validation.errors, consentReceipt, auditEntry });
    }
  } catch (e: unknown) {
    lastVerificationStatus = 'FAILED';
...
app.post('/oid4vp-present', async (req, res) => {
  try {
    ...
    if (!body.vp_token || !body.presentation_submission) {
      return res
        .status(400)
        .json({ ok: false, error: 'Missing vp_token or presentation_submission' });
    }

    if (!body.issuer_jwk) {
      return res
        .status(400)
        .json({ ok: false, error: 'Missing issuer_jwk (required for PoC verification)' });
    }
    ...
    if (kbJwtPart) {
      try {
        ...
        /* nonce extraction failed — validateSDJWTPresentation will catch it */
      }
    }
    ...
    const validation = await validateSDJWTPresentation({
      ...
      presentationSubmission:
        body.presentation_submission as import('@mitch/oid4vp').PresentationSubmission,
...
app.get('/health', (req, res) => {
  res.json({
    ...
    system: {
      ...
      nonce_store_entries:
        (nonceStore as unknown as { entries?: { size: number } }).entries?.size || 0,
    },
  });
});
...
app.post('/present', presentRouteLimiter, async (req, res) => {
  try {
    ...
    if (!rate.allowed) {
      ...
      console.warn(
        `[Provider] Rate limit exceeded (${requesterKind}). Resets in ${resetAfterSeconds}s.`
      );
    ...
    const firstPres = (presentation as Record<string, unknown[]>).presentations?.[0] as
      | Record<string, unknown>
      | undefined;
    const agePredicateId = 'age >= 18';

    // Check for ZKP Proof first
    const zkpProof = (firstPres?.zkp_proofs as Record<string, unknown> | undefined)?.[
      agePredicateId
    ];
    ...
    if (zkpProof) {
      ...
      isVerified =
        (firstPres?.proven_claims as Record<string, unknown> | undefined)?.[agePredicateId] ===
        true;
    }
    ...
    if (isVerified) {
      ...
      lastIssuer =
        (typeof issuerRef === 'string'
          ? issuerRef
          : (issuerRef as Record<string, string> | undefined)?.['issuer']) ||
        'Unknown Trusted Issuer';
    ...
    if (errName === 'ReplayDetectedError') {
      status = 409;
      errorKey = 'REPLAY_DETECTED';
    } else if (errName === 'TokenExpiredError') {
      status = 410;
      errorKey = 'TOKEN_EXPIRED';
    } else if (errName === 'ProofSignatureError') {
      status = 401;
      errorKey = 'INVALID_SIGNATURE';
    }

    lastVerificationStatus = 'FAILED';
    res
      .status(status)
      .json({ ok: false, error: errorKey, details: e instanceof Error ? e.message : String(e) });
  }
});
