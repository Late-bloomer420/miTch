...
export function VerifierPanel({ scenario, backendUrl, runNonce }: VerifierPanelProps) {
  const [panelState, setPanelState] = useState<'waiting' | 'verified' | 'failed' | 'offline'>(
    'waiting'
  );
  ...
  if (panelState === 'waiting') {
    const walletBaseUrl =
      (import.meta as unknown as Record<string, Record<string, string>>).env?.VITE_WALLET_URL ??
      'http://localhost:5174';
