    ...
    <div>
      ...
      {flowState === 'consent' && (
        ...
        >
          ...
          {scenario.verifierReceives.map((claim) => (
            <div
              key={claim.key}
              style={{ fontSize: 12, color: '#81c784', fontFamily: 'monospace', marginBottom: 4 }}
            >
              ✓ {claim.key}
            </div>
          ))}
          {scenario.blocked.length > 0 && (
            <>
              ...
              {scenario.blocked.map((field) => (
                <div
                  key={field}
                  style={{ fontSize: 11, color: '#444', fontFamily: 'monospace', marginBottom: 2 }}
                >
                  ✗ {field}
                </div>
              ))}
      ...
      {flowState === 'presenting' && (
        <div style={{ textAlign: 'center', padding: '16px 0', color: '#7986cb', fontSize: 13 }}>
          ...
          <div style={{ fontSize: 10, color: '#444' }}>SD-JWT VC + Key Binding JWT + OID4VP</div>
        </div>
      )}
