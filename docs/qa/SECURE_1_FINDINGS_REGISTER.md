...
| ID | File:line | Class | Description (1 line) | Severity | Confidence | Disposition |
|----|-----------|-------|----------------------|----------|------------|-------------|
| F-01 | `src/apps/wallet-pwa/src/services/WalletService.ts:1640` | `contract-mismatch` | `getIdentityPublicKey()` casts `AuditLog` to `{ publicKey?: CryptoKey }` but the field is `auditPublicKey`; always returns `null`. | med | high | fix |
| F-02 | `src/apps/wallet-pwa/src/services/WalletService.ts:601-624` | `swallowed-error` | `savePolicy()` fires `persistPolicy()` with `void …catch(console.error)` — policy write failures are logged but the caller receives no signal and the in-memory state diverges silently from persistent storage. | med | high | fix |
| F-03 | `src/packages/shared-crypto/src/webauthn.ts:602-604` | `false-success` | `WebAuthnService.verifyPresence()` returns `true` for any non-empty attestation string without doing any cryptographic verification; callers that rely on it believe a presence proof is verified when it is not. | high | high | fix |
| F-04 | `src/packages/policy-engine/src/geo-scope.ts:17` | `fail-closed-violation` | `isAllowedByGeoScope()` returns `true` (allow) when the country cannot be determined from the DID; comment explicitly says "fail-open for geo only", which violates the AskMI invariant that ambiguity → DENY. | med | high | fix |
