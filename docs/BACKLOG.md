...
**Update 2026-07-17 (ADOPT-0a/0b credential path; reconciled with the RC evidence rules 2026-08-26):** On exact revision `bb340038fdd4df98c6d2f2936f2a39ba7c872ac1` (#137), a local live probe observed issuer-mock issuance, selective disclosure of `age` + KB-JWT, verifier resolution through the configured development trust-list/JWKS path, and a consent receipt. The happy path returned `200 disclosedClaims:{age:24}`; the forged-key case returned `403 Signature verification failed`; the unknown-issuer case returned `403 untrusted_or_unresolvable_issuer`. `pnpm test` recorded 47/47 Turbo tasks on that revision. Evidence: [`qa/ADOPT_0AB_LIVE_VERIFICATION_2026-07-17.md`](qa/ADOPT_0AB_LIVE_VERIFICATION_2026-07-17.md). **Evidence boundary:** wallet storage APIs and wiring were repository-tested, but the live probe did not drive wallet-pwa browser UI, IndexedDB, or the QR handoff; the probe was deleted after the run and is historical internal evidence, not a rerunnable RC gate or external assurance. **Open/known gaps preserved:** only the age/liquor-store scenario had a matching real credential; the verifier-demo frontend path remained simulated; `pnpm dev` still failed on `@askmi/poc-hardened` at the time; issuer-mock returned 500 for a non-P-256 holder JWK; the OID4VCI fetch dropped `x-correlation-id`; and holder-binding extractable-key convergence remained open. ADOPT-1 remained planned, not implemented. PR #141 records later CORS/session/recovery hardening on separate exact revisions; independent security/cryptographic review, official EUDI conformance/interoperability, production key management, distributed verifier sessions, trust governance, and production operations remain open. Current authority: [`MATURITY_AND_LIMITATIONS.md`](MATURITY_AND_LIMITATIONS.md) and [`RELEASE_CANDIDATE_CHECKLIST.md`](RELEASE_CANDIDATE_CHECKLIST.md).
