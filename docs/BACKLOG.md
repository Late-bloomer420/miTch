...
| CL-01 | 🔴 P0 | ⏳ | Demo-Aussagen bereinigen: „CIR 82%“, grüne Compliance-Häkchen, Interoperabilitäts-Flaggen, Absolutformulierungen („cannot“, „mathematically“), Kennzeichnung als Simulation (FNV-Hash statt SHA-256, `Date.now()` im Nullifier), reale Organisationsnamen als fiktive Verifier | Jede öffentliche Aussage ist in `docs/05-business/marketing/06-claim-evidence-map.md` mit Beleg eingetragen; keine Aussage widerspricht `MATURITY_AND_LIMITATIONS.md` |
| CL-02 | 🟡 P1 | ⏳ | Kostenmodell der Demo: belegte Werte (IBM-Report als Annahme mit Ausgabe und USD, HSM- und CDN-Listenpreise, Beispielpreis Verifizierung) mit Quelle und Stand kennzeichnen; alle übrigen Posten als frei einstellbare Annahmen; keine „Ersparnis in %“ als Überschrift | Jeder Wert trägt ein Etikett mit Quelle und Stand oder ist als Annahme einstellbar |
