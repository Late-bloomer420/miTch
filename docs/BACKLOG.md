...
**Update 2026-07-17 (SECURE-2 + ADOPT-0a/0b real credential path — live verifiziert):** master jetzt `bb34003`/**PR #137** (war `e9bec3c`/#120 in den älteren Stand-Zeilen). Gemerged seit SECURE-1: **#133** (`turbo dev dependsOn ^build`), **#134** (SECURE-2 internes Evidence-Pack + runnable Harness), **#135 (ADOPT-0a)** echte SD-JWT-VC-Ausstellung + Speicherung, **#136 (ADOPT-0b)** echte Credential-Präsentation aus der gespeicherten VC, **#137** Korrektur-Fix (Verifier dekodiert SD-JWT-Disclosures; Issuer stellt den angefragten Claim `age` aus — beide Bugs waren unit-grün aber e2e kaputt). **ERLEDIGT (live verifiziert 2026-07-17 gegen frische current-`master`-Server):** Der echte Pfad Wallet→issuer-mock→Verifier `/oid4vp-present` funktioniert end-to-end — Ausstellung `vc+sd-jwt` (holder-bound `cnf`) → selektive Offenlegung nur `age` + KB-JWT → Verifier löst den Issuer-Key **autoritativ** über die Trust-List/JWKS auf und dekodiert die Disclosures. Drei Live-Fälle grün: Happy-Path (`200 disclosedClaims:{age:24}`), **Circular-Verification geschlossen** (Angreifer-signierte Cred mit vertrautem `iss` + eigenem `issuer_jwk` → `403 Signature verification failed`, wallet-gelieferter Key ignoriert), **fail-closed Trust-Gate** (`did:web:evil.example` → `403 untrusted_or_unresolvable_issuer`). `pnpm test` → **47/47 Turbo-Tasks** grün. Real-Facts-Fund: auf `:3004`/`:5174` liefen 3 Tage alte Zombie-Prozesse (pre-merge Code) — vor jeder Live-Demo den laufenden Prozess als current bestätigen. Evidenz: [`qa/ADOPT_0AB_LIVE_VERIFICATION_2026-07-17.md`](qa/ADOPT_0AB_LIVE_VERIFICATION_2026-07-17.md). **OFFEN / KNOWN GAP:** (a) nur das `age`/liquor-store-Szenario läuft mit echter Credential end-to-end — Doctor/EHDS/Pharmacy fallen korrekt fail-closed ("no matching credential"); (b) Browser-UI-Click-through (wallet-pwa `:5174` HTTPS + IndexedDB + In-Browser-OID4VCI/QR) **nicht** automatisiert gefahren — nur der HTTP-Pfad; (c) `pnpm dev` scheitert weiterhin an `@askmi/poc-hardened` (`ERR_MODULE_NOT_FOUND`) → Demo einzeln bzw. `--filter=!@askmi/poc-hardened` starten; (d) verifier-demo **Frontend** (`:5175`) `/wallet-present` bleibt die **simulierte** Wallet (ephemere Issuer-Keys), nicht der echte ADOPT-Pfad; (e) Carry-over-Minors: issuer-mock 500 statt 400 bei nicht-P-256 Holder-JWK, `x-correlation-id` beim OID4VCI-Fetch fallen gelassen. **Nächster Track:** ADOPT-1 (`@askmi/connect` "Sign in with AskMI" Kit) — Re-Brainstorm auf realer Faktenbasis, gedrafteter Spec nur als „old truth".
