...
| S-11 | 🟡 P1 ⏳ | **Enforcement-Layer unterschützt Health/Professional-Vokabular** ([Issue #122](https://github.com/Late-bloomer420/miTch/issues/122)) — `bloodGroup`/`allergies`/`emergencyContacts`/`medication`/`dosageInstruction`/`refillsRemaining`/`role`/`licenseId` fehlen im Enforcement-`LAYER_MAP` → default `WELT`, d.h. ein WELT-Verifier besteht den Layer-Check für sensible Claims. Aufgedeckt durch G-140 (#119/#120): Visibility zeigt sie korrekt als `high`. **Kein Ein-Zeilen-Fix** — Map-Erweiterung änderte in PR-A 23 EHDS-Verdicts; Enforcement-Semantik + Demo-Regeln + Tests müssen gemeinsam entworfen werden. Enforcement **nicht** ohne diese bewusste Review anfassen. | Under-enforcement: Low-Layer-Verifier erhält Layer-2-Gesundheitsdaten ohne Layer-Violation |
