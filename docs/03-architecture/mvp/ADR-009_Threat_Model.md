...
| I-10 | OID4VP Verifier | Credential-Authentizität nur strukturell geprüft — Issuer-Signatur nie kryptografisch verifiziert | Smart Policy Engine | **SECURE-1 F-14:** `verifyAuthorizationResponse()` ist jetzt async und verdrahtet `validateSDJWTVC` + `validateKeyBindingJWT` aus `@askmi/shared-crypto`. Neue Opts `verifyCredentialSignatures`, `resolveIssuerKey`, `expectedAudience`; neues Ergebnisfeld `signaturesVerified`. Fail-closed bei fehlendem Resolver, Null-/falschem Key, KB-JWT-Mismatch oder unbekanntem Format. Opt-in; strukturelle Prüfung rückwärtskompatibel (commit 703f06d). | belegt | `src/packages/oid4vp-verifier/src/__tests__/response-verifier.crypto.test.ts` — 7 TDD-Crypto-Tests (RED→GREEN), 60 Tests gesamt grün |
...
| Mind. 12 Mitigations mit Test-Vektoren | **Erfüllt** | 30 STRIDE-Einträge (22 Baseline + I-8/I-9/D-4 Sprint 1 & Proof-Randomization + T-7/T-8/T-9/T-10/I-10 SECURE-1; I-6 wurde ergänzt, nicht neu hinzugefügt), alle mit Evidenz-Status "belegt" und verifizierter Testdatei-/Sprint-Referenz |
